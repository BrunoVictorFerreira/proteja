...
LEI Nº 13.709, DE 14 DE AGOSTO DE 2018.
    ...
    <div class="cadastro" style="padding-right: 15px;width: 50vw;margin-bottom: 50px;box-shadow: 10px 10px 10px rgba(0,0,0,.2)">
      ...
      <div class="row">
        ...
        <div class="col-sm-12 col-lg-8">
          <form action="{{route('alterar_senhaa')}}" method="POST" style="margin-top: 10px">
            {{ csrf_field() }}
            @if ($errors->any())
            ...
            <div class="form-group">
              ...
              <input type="password" class="form-control" id="senha" name="senha_antiga" placeholder="" required>
              <div class="input-group-prepend">
                  <div class="input-group-text" style="background-color: white;color: #148b7e;cursor: pointer" id="visualizar_1"><i class="icon-eye"></i></div>
                </div>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="input-group-prepend">
                  <div class="input-group-text" style="background-color: white;color: #148b7e;cursor: pointer" id="visualizar_2"><i class="icon-eye"></i></div>
                </div>
              </div>
              <small style="color: darkred">Mínimo de 8 caracteres, contendo letras e números</small>
            </div>
            <div class="form-group">
              <label for="cpf">Repita a nova senha</label>
              <div class="input-group mb-2">
                <input type="password" class="form-control" id="senha_nova_repete" placeholder="" required>
                <div class="input-group-prepend">
                  <div class="input-group-text" style="background-color: white;color: #148b7e;cursor: pointer" id="visualizar_3"><i class="icon-eye"></i></div>
                </div>
              </div>
            </div>

            <button type="submit" class="btn btn-success center" id="btn-cadastrar">Alterar senha</button>
            @isset($success)
                @if($success == '1')
                  <div class="alert alert-success text-center">Senha Alterada com sucesso!</div>
                  <a href="{{route('painel')}}" class="btn btn-sm btn-success mb-2" style="margin-left: auto;margin-right: auto;display: block;width: 150px">Voltar para o painel</a>
                @else
                    <div class="alert alert-danger text-center">Senha antiga está incorreta</div>
                    <a href="{{route('painel')}}" class="btn btn-sm btn-success mb-2" style="margin-left: auto;margin-right: auto;display: block;width: 150px">Voltar para o painel</a>
  ...
  <script type="text/javascript">
    ...
    $('#visualizar_3').on("click", ()=>{
      if(ident == 0){
        $('#senha_nova_repete').attr('type', 'text');
        ident++;
      }else{
        $('#senha_nova_repete').attr('type', 'password');
        ident--;
      }
    });

    $('#btn-cadastrar').on("click", (event) => {
      if(!($('#senha_nova_repete').val() == $('#senha_nova').val())){
